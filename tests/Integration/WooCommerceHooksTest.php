...
use CustomKings\CKPP_WooCommerce_Hooks;

class WooCommerceHooksTest extends IntegrationTestCase
{
    protected $woocommerce_hooks;
    protected $test_data;
    protected $user_id;
    ...
    protected function setUp(): void
    {
        ...
        $this->woocommerce_hooks = new CKPP_WooCommerce_Hooks();
    ...
    public function test_add_to_cart_validation()
    {
        ...
        $valid = $this->woocommerce_hooks->validate_add_to_cart(
            true,
            $product->get_id(),
            1,
            [],
        ...
        $invalid = $this->woocommerce_hooks->validate_add_to_cart(
            true,
            $product->get_id(),
            1,
            [],
    ...
    public function test_add_cart_item_data()
    {
        ...
        $cart_item_data = [];
        $form_data = [
            'ckpp_design_id' => $design_id,
            'ckpp_design_data' => json_encode($design_data)
        ];
        
        // Call the method
        $result = $this->woocommerce_hooks->add_cart_item_data($cart_item_data, $product->get_id(), 0, $form_data);
        
        // Verify the result
        $this->assertArrayHasKey('ckpp_design_id', $result);
        $this->assertArrayHasKey('ckpp_design_data', $result);
        $this->assertEquals($design_id, $result['ckpp_design_id']);
        $this->assertEquals($design_data, json_decode($result['ckpp_design_data'], true));
    }
    ...
    public function test_get_item_data()
    {
        ...
        // Call the method
        $result = $this->woocommerce_hooks->get_item_data([], $cart_item);
    ...
    public function test_order_item_display_meta()
    {
        ...
        // Call the method
        $result = $this->woocommerce_hooks->order_item_display_meta($item_data, $item);
        
        // Verify the result
        $this->assertIsArray($result);
        $this->assertCount(1, $result);
        $this->assertEquals('Design', $result[0]['key']);
        $this->assertStringContainsString('Custom Design', $result[0]['display']);
    }
}
