...
class PluginTest extends UnitTestCase
{
    ...
    {
        // Test admin hooks
        $this->assertEquals(10, has_action('admin_enqueue_scripts', [$this->plugin->admin, 'enqueue_admin_scripts']));
        $this->assertEquals(10, has_action('admin_menu', [$this->plugin->admin, 'add_admin_menu']));
        $this->assertEquals(10, has_action('admin_init', [$this->plugin->admin, 'register_settings']));
    ...
    public function test_run()
    {
        ...
        Functions\expect('add_action')
            ->once()
            ->with('plugins_loaded', [$this->plugin, 'load_plugin_textdomain']);
            
        Functions\expect('add_action')
            ->once()
            ->with('init', [$this->plugin, 'init']);
